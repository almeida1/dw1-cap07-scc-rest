...
class REQ02ConsultarClienteTests {
	...
	void ct01_quando_consulta_todos_retorna_a_lista_de_clientes_cadastrados() {
		...
		// Dado que o id do cliente esta cadastrado e o login de usuario esta cadastrado configura o header com username e senha
		...
		ResponseEntity<List<Cliente>> resposta3 = testRestTemplate.exchange("/api/v1/clientes", HttpMethod.GET,
				httpEntity2, new ParameterizedTypeReference<List<Cliente>>() {
				});
		// Entao
	...
	void ct02_quando_consulta_por_id_cadastrado_retorna_detalhes_do_cliente() {
		...
		ResponseEntity<Cliente> resposta2 = testRestTemplate.exchange("/api/v1/clientes/id/{id}", HttpMethod.GET,
				httpEntity2, Cliente.class, 1L);
...
//		HttpEntity<ApplicationUser> httpEntity = new HttpEntity<>(user);
//		ResponseEntity<String> resposta1 = testRestTemplate.exchange("/users/sign-up", HttpMethod.POST, httpEntity,
//				String.class);
//		assertEquals(HttpStatus.OK, resposta1.getStatusCode());
//		// tenta autenticar o usuario para obter o token
//		ResponseEntity<String> resposta2 = testRestTemplate.exchange("/login", HttpMethod.POST, httpEntity,
//				String.class);
//		assertEquals(HttpStatus.OK, resposta2.getStatusCode());
//		// armazena o token no header do post
//		HttpHeaders headers = resposta2.getHeaders();
//		HttpEntity<?> httpEntity2 = new HttpEntity<>(headers);

		// Quando solicita consulta por cpf
		ResponseEntity<Cliente> resposta3 = testRestTemplate.exchange("/api/v1/clientes/{cpf}", HttpMethod.GET,
				null, Cliente.class, "99504993052");
		// Entao retorna os detalhes do cliente

		assertEquals("200 OK", resposta3.getStatusCode().toString());
		assertNotNull(resposta3.getBody());
		assertEquals("Miguel Soares", resposta3.getBody().getNome());
	}
}
